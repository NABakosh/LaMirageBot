...
// Find line with "async function handleMessage"
const handleMessageLine = lines.findIndex(line => line.includes('async function handleMessage'));
console.log(`Found handleMessage at line ${handleMessageLine + 1}`);

// Insert validation functions before handleMessage
...
lines.splice(handleMessageLine, 0, ...validationCode.split('\n'));

// Now find and update asking_name handler
const askingNameLine = lines.findIndex((line, idx) => idx > handleMessageLine && line.includes("if (conversation.stage === 'asking_name')"));
console.log(`Found asking_name at line ${askingNameLine + 1}`);

// Find the end of asking_name block (next "if (conversation.stage")
let askingNameEnd = askingNameLine + 1;
let braceCount = 0;
let started = false;
for (let i = askingNameLine; i < lines.length; i++) {
\tif (lines[i].includes('{')) {
\t\tbraceCount++;
\t\tstarted = true;
\t}
...
console.log(`asking_name block ends at line ${askingNameEnd + 1}`);

// Replace asking_name handler
const newAskingName = `\tif (conversation.stage === 'asking_name') {
...
\t\t\tconversation.client_phone = null
...
lines.splice(askingNameLine, askingNameEnd - askingNameLine + 1, ...newAskingName.split('\n'));

// Find and update asking_phone handler
const askingPhoneLine = lines.findIndex((line, idx) => idx > askingNameLine && line.includes("if (conversation.stage === 'asking_phone')"));
console.log(`Found asking_phone at line ${askingPhoneLine + 1}`);

// Find end of asking_phone block
let askingPhoneEnd = askingPhoneLine + 1;
braceCount = 0;
started = false;
for (let i = askingPhoneLine; i < lines.length; i++) {
\tif (lines[i].includes('{')) {
\t\tbraceCount++;
\t\tstarted = true;
\t}
...
console.log(`asking_phone block ends at line ${askingPhoneEnd + 1}`);

// Replace asking_phone handler
const newAskingPhone = `\tif (conversation.stage === 'asking_phone') {
...
lines.splice(askingPhoneLine, askingPhoneEnd - askingPhoneLine + 1, ...newAskingPhone.split('\n'));

// Write back
fs.writeFileSync('main.js', lines.join('\r\n'), 'utf8');

console.log('✅ Successfully integrated Gemini AI validation!');
console.log('- Added validateNameWithGemini()');
console.log('- Added validatePhoneWithGemini()');
console.log('- Updated asking_name handler to use AI validation');
console.log('- Updated asking_phone handler to use AI validation');
